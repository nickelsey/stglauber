...
{
  std::cout  << "StGlauberUtilities  Initialize StGlauberUtilities" << std::endl;
  ...
  std::cout  << "StGlauberUtilities  Set impact parameter distributions: "
...
Double_t StGlauberUtilities::GetImpactParameter() const
{
  ...
  if( mDebug > 0 ){
    std::cout  << "StGlauberUtilities::GetImpactParameter  Generate random b = "
         << b
         << " (fm)" << std::endl;
  }
...
Double_t StGlauberUtilities::GetTheta() const
{
  ...
  if( mDebug > 0 ){
    std::cout  << "StGlauberUtilities::GetTheta  Generate random theta = " << theta << std::endl ;
  }
...
Double_t StGlauberUtilities::GetPhi() const
{
  ...
  if( mDebug > 0 ){
    std::cout  << "StGlauberUtilities::GetPhi  Generate random phi = " << phi << std::endl;
  }
...
{
  mDebug = debug ;
  std::cout  << "StGlauberUtilities::SetDebug  Set debug level = " << mDebug << std::endl;
}
